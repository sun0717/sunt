...
const getCanvasData = async (
  ...
  const measureTextSize = (
    ...
    content.forEach((item) => {
      // fontBoundingBoxAscent 是 baseline 到顶部的距离
      // fontBoundingBoxDescent 是 到底部的距离
      const {
        width: textWidth,
    ...
    // 如果有旋转，用 sin 、cos 算出旋转后的宽高
    const angle = (rotate * Math.PI) / 100;
  ...
  function drawImage() {
    return new Promise<{ width: number; height: number; base64Url: string }>(
      (resolve) => {
        ...
        // refererPolicy 设置 no-referrer 是不携带 referer , 都是安全相关的
    ...
    getCanvasData(mergedOptions).then(({ base64Url, width, height }) => {
      ...
      if (container) {
        ...
        mutationObserver.current = new MutationObserver((mutations) => {
          // 判断水印是否删除是通过判断是否修改了 watermark 节点的属性
          // 是否增删了 watermark 节点
